...
import {Migrateable} from  "./Migrator.sol";
import {Terminatable} from "./EarlyTermination.sol";
import {ITokenLockerEvents} from "@governance/TokenLocker.sol";
...
contract TokenLocker is IncentiveCurve, ITokenLockerEvents, AccessControlEnumerable, Migrateable, Terminatable {
        ...
        onlyRole(COMPOUNDER_ROLE)
    {
        ...
        depositToken.safeTransferFrom(_msgSender(), address(this), total);
    }
    ...
    ///@dev Technically LSD is going to call himself, we could have touched the lock here directly
    ///@dev however we decided this contract needs to be agnostic to what happens in the lsd
    ///@notice The penalty collected into a wallet: penaltyBeneficiary
    ...
    function _increaseAmount(uint192 _amountNewTokens) internal {
        ...
        Lock memory lock = lockOf[sender];
        ...
        // transfer deposit tokens and mint more veTokens
        depositToken.safeTransferFrom(sender, address(this), _amountNewTokens);
